...
unsigned long parse_hex_str(const char *s, unsigned int max_len) {
    unsigned long r = 0;
    unsigned long i;
...
unsigned long cpio_len_next(unsigned long len, const void *prev,
                            const void *next) {
...
int cpio_parse_header(const struct cpio_header *archive, unsigned long len,
    ...
    filename_length =
        parse_hex_str(archive->c_namesize, sizeof(archive->c_namesize));
    ...
    if (filename_length >= sizeof(CPIO_FOOTER_MAGIC) &&
        cpio_strncmp(filename, CPIO_FOOTER_MAGIC, sizeof(CPIO_FOOTER_MAGIC)) ==
            0) {
        return 1;
    }
    ...
    data = (void *)align_up((unsigned long)archive +
                            ...
                            CPIO_ALIGNMENT);
    next = (struct cpio_header *)align_up((unsigned long)data + filesize,
                                          CPIO_ALIGNMENT);
 ...
 * Runs in O(n) time.
 */
const void *cpio_get_entry(const void *archive, unsigned long len, int n,
 ...
 * Runs in O(n) time.
 */
const void *cpio_get_file(const void *archive, unsigned long len,
                          const char *name, unsigned long *size) {
    const struct cpio_header *header = archive;
    struct cpio_header_info header_info;
    ...
    while (1) {
        ...
        if (cpio_strncmp(header_info.filename, name, (unsigned long)(-1)) ==
            0) {
            break;
        }
        len = cpio_len_next(len, header, header_info.next);
...
void cpio_ls(const void *archive, unsigned long len, char **buf,
